...
//       Set to 1 to freeze this edge's stencil: yield_stress -> 1e30,
...
// ---------------------------------------------------------------------------
class StressPlasticDiscreteShellBendingModifier final : public FiniteElementConstraint
{
    static constexpr U64 ModifierUID = 35;

    SimSystemSlot<StressPlasticDiscreteShellBending> m_spdsb;
  ...
  public:
    ...
    {
        m_spdsb = require<StressPlasticDiscreteShellBending>();
    }
    ...
    void do_init(FiniteElementAnimator::FilteredInfo& info) override
    {
        ...
        const auto& h_stencils = m_spdsb->h_stencils;
        ...
        for(SizeT i = 0; i < h_stencils.size(); ++i)
        {
            ...
            for(IndexT g = 0; g < (IndexT)m_geo_infos.size(); ++g)
            {
                ...
                // Compute local edge indices and look up in edge map
        ...
        d_initial_theta_bars.resize(m_spdsb->h_theta_bars.size());
        d_initial_theta_bars.view().copy_from(m_spdsb->h_theta_bars.data());
    }
        ...
        ParallelFor()
            ...
            .apply(d_freeze_ids.size(),
                   [ids      = d_freeze_ids.cviewer().name("freeze_ids"),
                    ...
                    ys       = m_spdsb->yield_stresses.viewer().name("yield_stresses"),
                    bs       = m_spdsb->bending_stiffnesses.viewer().name("bending_stiffnesses"),
                    tbs      = m_spdsb->theta_bars.viewer().name("theta_bars"),
...
REGISTER_SIM_SYSTEM(StressPlasticDiscreteShellBendingModifier);
}  // namespace uipc::backend::cuda
