...
{
constexpr U64 StressPlasticDiscreteShellBendingModifierUID = 35;

REGISTER_CONSTITUTION_UIDS()
{
    using namespace builtin;
    list<UIDInfo> uids;
    uids.push_back(UIDInfo{.uid  = StressPlasticDiscreteShellBendingModifierUID,
                           .name = "StressPlasticDiscreteShellBendingModifier",
                           .type = string{builtin::Constraint}});
    return uids;
};

StressPlasticDiscreteShellBendingModifier::StressPlasticDiscreteShellBendingModifier(
    const Json& config) noexcept
    : m_config(config)
{
}

void StressPlasticDiscreteShellBendingModifier::apply_to(geometry::SimplicialComplex& sc) const
{
    Base::apply_to(sc);
...
Json StressPlasticDiscreteShellBendingModifier::default_config()
{
    return Json::object();
}

U64 StressPlasticDiscreteShellBendingModifier::get_uid() const noexcept
{
    return StressPlasticDiscreteShellBendingModifierUID;
}
}  // namespace uipc::constitution
