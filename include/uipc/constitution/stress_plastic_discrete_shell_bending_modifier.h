 ...
 * raises the bending stiffness of a StressPlasticDiscreteShellBending mesh.
 *
 * Apply this alongside StressPlasticDiscreteShellBending. At any point during
 * the simulation set the per-mesh meta attribute "cancel_plastic" to 1
 * (e.g. via `view(sc.meta().find("cancel_plastic"))[0] = 1`).  The
 * backend will then set yield_stress = 1e30 for every stencil belonging to
 ...
 */
class UIPC_CONSTITUTION_API StressPlasticDiscreteShellBendingModifier final
    : public Constraint
{
    using Base = Constraint;

  public:
    StressPlasticDiscreteShellBendingModifier(const Json& config = default_config()) noexcept;
